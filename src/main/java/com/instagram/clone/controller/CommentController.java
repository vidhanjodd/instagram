...
public class CommentController {
    ...
    // ── Form submit from feed card (redirects back to post) ──────────────────
    @PostMapping("/add")
    public String create(@ModelAttribute CommentRequest request) {
        if (request.getParentId() == null) {
            commentService.createTopLevelComment(request);
    ...
    // ── AJAX JSON endpoint used by the post popup in feed.html ───────────────
    @PostMapping("/add/json")
    @ResponseBody
    public ResponseEntity<CommentResponse> createJson(@RequestBody CommentRequest request) {
        try {
